...
$('#prediction-form').addEventListener('submit',e=>{
  ...
  const a=vector(cd,cs*(.6+.4*resist)), b=vector((wd+180)%360,ws*.198*resist), c=vector((pd+180)%360,ph*.32*resist);
  ...
  const radius=Math.min(3,.25+ph*.35+ws*.035+hours*.025), probability=Math.max(35,Math.round(88-ph*10-ws*1.2-hours*.7));
  ...
  const route=curvedRoute(chosen,end,heading);if(!route.length)route.push([chosen.lat,chosen.lng]);const safeEnd=route.at(-1);L.polyline(route,{color:'#0968e8',weight:4,dashArray:'8 9',lineCap:'round'}).addTo(resultMap);
